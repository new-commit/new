...
public class Pass1asm {
    ...
    public static void main(String[] args) {
        ...
        try {
            one.parseFile();
            one.printLocc(); // Print locc.txt after processing
        } catch (Exception e) {
    ...
    public void parseFile() throws Exception {
        ...
        BufferedWriter bwLocc = new BufferedWriter(new FileWriter("locc.txt")); // New file for lc with each line
        OPtable lookup = new OPtable();
        System.out.println("\nIntermediate Code");

        while ((line = br.readLine()) != null) {
            // Append lc to the end of each line and write to locc.txt
            String lineWithLC = line.trim() + " " + lc; // Append lc to the end of the line
            bwLocc.write(lineWithLC + "\n"); // Write to locc.txt
        ...
        bw.close();
        bwLocc.close(); // Close locc.txt after writing
        printIC();
        printSYMTAB();
        PrintLITTAB();
        printPOOLTAB();
    ...
    void printLocc() throws IOException {
        BufferedReader br = new BufferedReader(new FileReader("locc.txt"));
        System.out.println("\nContents of locc.txt (Line with Location Counter):");
        String line = br.readLine();
        while (line != null) {
            System.out.println(line);
            line = br.readLine();
